...
const activeBots    = new Map() // number -> { child, version, connected }
...
function spawnBot(number, version, isRestore) {
    const existing = activeBots.get(number)
    if (existing) {
    ...
    activeBots.set(number, entry)
    if (!isRestore) pendingCodes.set(number, { status: 'pending', code: null, error: null })

    child.stdout.on('data', d => {
        const text = d.toString()
        process.stdout.write(`[bot:${number}] ${text}`)

        const codeMatch = text.match(/🔐 CODE\s*:\s*([A-Za-z0-9-]+)/)
        if (codeMatch && !entry.connected) {
            pendingCodes.set(number, { status: 'ready', code: codeMatch[1], error: null })
        }
        if (/✅[^\n]*connect/i.test(text)) {
    ...
    child.stderr.on('data', d => process.stderr.write(`[bot:${number}:err] ${d}`))

    child.on('exit', code => {
        console.log(`⚠️ Bot +${number} (${version}) arrêté (code ${code})`)
        if (activeBots.get(number) === entry) activeBots.delete(number)
        if (!entry.connected) {
            pendingCodes.set(number, { status: 'error', code: null, error: `Le bot s'est arrêté au démarrage (code ${code}). Regarde les logs serveur.` })
        } else {
            ...
            pendingCodes.set(number, { status: 'error', code: null, error: `Le bot a été déconnecté (logout WhatsApp). Reconnecte-toi depuis le site si besoin.` })
        }
    })
...
export function isWebpairManaged(number) { return activeBots.has(number) }
export function getWebpairBot(number) { return activeBots.get(number) }
