...
function saveSession(number) {
    try {
        ...
        if (!list.includes(number)) { list.push(number); fs.writeFileSync(SESSIONS_FILE, JSON.stringify(list, null, 2)) }
    } catch (e) {}
}
    ...
    sock.ev.on('connection.update', async function(update) {
        ...
        if (connection === 'open') {
            ...
            if (!msgHandlerAttached) {
                ...
                try {
                    const { handleMessage } = await import('./utils/handler.js')
                    ...
                    console.error('Handler import error:', e.message)
                    // Fallback : essayer index.js
                    try {
                    ...
                    } catch (e2) { console.error('Fallback import error:', e2.message) }
                }
            }
        }
        if (connection === 'close') {
...
async function restoreSessions() {
    ...
    for (const number of list) {
        ...
        try { await startSocket(number, true); await new Promise(r => setTimeout(r, 1500)) } catch (e) {}
    }
}
