// webpair.js — AKANE MD v1 + v2 : FAÇADE WEB à déployer sur Render.
// Ne fait tourner AUCUN bot ici : sert la page de pairing et relaie /pair, /disconnect,
// /code/:number et /stats vers le vrai backend (runner-server.js) hébergé sur Téo Héberg.
// C'est ce qui évite les problèmes de mémoire — Render ne porte que l'UI, légère.
...
// URL publique du runner sur Téo Héberg (ex: https://ton-app.teoheberg.fr) et la même
// clé que RUNNER_KEY côté runner. Sans ces deux variables d'env sur Render, rien ne
// peut être relayé. Si v1 est désactivé côté runner (V1_ENABLED=false), l'erreur
// remontera simplement dans showError() au moment du /pair — pas besoin de le savoir
// ici à l'avance.
const RUNNER_URL = process.env.RUNNER_URL || ''
const RUNNER_KEY = process.env.RUNNER_KEY || ''
if (!RUNNER_URL || !RUNNER_KEY) {
    console.error('❌ RUNNER_URL et/ou RUNNER_KEY manquantes — définis ces variables d\'env sur Render (même RUNNER_KEY que sur Téo Héberg).')
}

// Relaie une requête vers le runner, avec la clé, et renvoie sa réponse JSON telle quelle.
async function callRunner(method, urlPath, body) {
    const r = await fetch(`${RUNNER_URL}${urlPath}`, {
        method,
...
app.post('/pair', async function(req, res) {
    try {
        res.json(await callRunner('POST', '/pair', req.body))
    } catch (e) {
        res.json({ error: 'Backend injoignable (Téo Héberg) : ' + e.message })
    }
})

app.post('/disconnect', async function(req, res) {
    try {
        res.json(await callRunner('POST', '/disconnect', req.body))
    } catch (e) {
        res.json({ error: 'Backend injoignable (Téo Héberg) : ' + e.message })
    }
})
...
app.get('/code/:number', async function(req, res) {
    try {
        ...
        res.json(await callRunner('GET', `/code/${encodeURIComponent(req.params.number)}?version=${version}`))
    } catch (e) {
        res.json({ status: 'error', error: 'Backend injoignable (Téo Héberg)' })
    }
})

app.get('/stats', async function(req, res) {
    try {
        res.json(await callRunner('GET', '/stats'))
    } catch (e) {
        res.json({ connected: 0 })
    }
})

// Health check de la façade elle-même (utile pour Render) — ne dépend pas du runner.
app.get('/health', function(req, res) { res.json({ status: 'ok', role: 'frontend' }) })
...
<div class="terminal">
  ...
  <div class="term-body">
    ...
    <!-- VERSION TABS : les deux sont cliquables ici ; si v1 est désactivé côté runner
         (V1_ENABLED=false), l'erreur remonte via showError() au clic sur "obtenir le code" -->
...
app.listen(PORT, function() {
    console.log('AKANE MD Web Pair (façade Render) -> http://localhost:' + PORT)
    console.log(RUNNER_URL ? `↳ relaie vers le runner : ${RUNNER_URL}` : '⚠️ RUNNER_URL non définie, le pairing ne fonctionnera pas')
})
